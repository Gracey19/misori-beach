      ...
      >
        ...
        <nav className="flex flex-col gap-4 text-center mt-8 ">
          {['Home', 'Accommodations', 'Dining', 'The Lake', 'Contact us'].map((item) => {
            ...
            let myHref = `#${item.toLowerCase().replace(' ', '')}`;
    
            // 2. If it's Home, go to "/"
            if (item === 'Home') myHref = "/";
    
            // 3. If it's Accommodations, go to "/accommodations"
            if (item === 'Accommodations') myHref = "/accommodations";

            return (
              <a
                key={item}
                href={myHref} // Use our new address
                onClick={() => setIsMenuOpen(false)}
